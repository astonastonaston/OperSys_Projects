...
__global__ void mykernel(int input_size) {
  ...
  if (threadIdx.x == 0) {
    ...
    // printf("after prog 0, pagefault number is %d\n", pagefault_num);
  }
  __syncthreads();
  if (threadIdx.x == 1) {
    user_program(&vm, input, results, input_size);
    // printf("after prog 1, pagefault number is %d\n", pagefault_num);
  }
  __syncthreads();
  if (threadIdx.x == 2) {
    user_program(&vm, input, results, input_size);
    // printf("after prog 2, pagefault number is %d\n", pagefault_num);
  }
  __syncthreads();
  if (threadIdx.x == 3) {
   user_program(&vm, input, results, input_size);
    // printf("after prog 3, pagefault number is %d\n", pagefault_num);
  }
  __syncthreads();
}
...
int main() {
  ...
  // write GM results to output file
